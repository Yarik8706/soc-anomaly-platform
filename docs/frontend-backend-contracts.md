...
`mode`: `report|metrics|report+metrics|full|dry-run`. Для `week` рассчитываются 7
доступных дат по опорную включительно, для `month` — 30, а `range` и `all` анализируют
каждую доступную дату. Этапы, исключённые режимом, получают status `skipped`, а не
остаются `pending`.
...
`context` может содержать `time_range`, `most_active_hours`, массивы `ip_addresses`,
`processes`, `event_names`, `users`, `hosts`, `source_files`, `original_user`, подробное
описание и полный набор raw/normalized/rank score; каждое поле опционально.
...
interface ReportFileRead {
  format: "markdown" | "pdf" | "context" | string;
  filename: string;
  size: number;
  url: string;
}
...
Все роли. `format_name`: `markdown|pdf|context`. `context` — подробная CSV по всем
сущностям. Ответ — файл с `Content-Disposition`.
...
Новые запуски рассчитывают метрики автоматически по полным score-распределениям.
`distribution_statistics` содержит mean/std/median/p90/p95/p99/tail gap/tail ratio и
severity counts; stability grid варьирует contamination, n_neighbors, n_estimators,
max_samples и random_state для нескольких K. Для исторических запусков без score CSV
сохраняется fallback-сравнение с предыдущим запуском.
